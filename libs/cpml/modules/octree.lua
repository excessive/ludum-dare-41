...
function Octree:get_colliding_frustum(checkFrustum)
	return self.rootNode:get_colliding_frustum(checkFrustum)
end
...
-- @param results List results
-- @return table Objects that intersect with the specified frustum
function OctreeNode:get_colliding_frustum(checkFrustum, results)
	results = results or {}

	-- Are the input bounds at least partially in this node?
	if not intersect.aabb_frustum(self.bounds, checkFrustum) then
		return results
	end

	-- Check against any objects in this node
	for _, object in ipairs(self.objects) do
		if intersect.aabb_frustum(object.bounds, checkFrustum) then
			table.insert(results, object.data)
		end
	end
	...
	-- Check children
	for _, child in ipairs(self.children) do
		results = child:get_colliding_frustum(checkFrustum, results)
	end
